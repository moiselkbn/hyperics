...
const { classes, coursesByClassId } = await scrapeAllClasses({
  limit,
  onProgress: (label, i, total) => console.log(`[${i + 1}/${total}] ${label}`),
});

const elapsed = ((Date.now() - start) / 1000).toFixed(1);
const totalCourses = Object.values(coursesByClassId).reduce((sum, c) => sum + c.length, 0);

console.log(`\n${classes.length} classes disponibles, ${Object.keys(coursesByClassId).length} scrapées, ${totalCourses} cours au total.`);
